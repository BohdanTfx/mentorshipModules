...
public class MenteeJpaDao extends GenericJpaDao<Mentee, Long>
    ...
    @Override
    public List<Mentee> getMenteesWithoutMentorByLocation(
        ...
        query.select(root)
                .where(builder.and(builder.equal(
                        root.get("mentorshipProgram").get("location"),
                        location), builder.isNull(root.get("mentor"))));
    ...
    public List<Mentee> filterMentees(final boolean descendingOrder,
        ...
        Root<Mentee> root = query.from(Mentee.class);
        query.select(root)
                .orderBy(builder.desc(root.get("mentorshipDuration")));
        return getEntityManager().createQuery(query)
                .setFirstResult(startPosition).setMaxResults(itemsNumber)
                .getResultList();
    }
    ...
    public Double getFinishStatistic(final Date startDate, final Date endDate,
        ...
        queryForFiltering.select(builder.countDistinct(filteredRoot))
                .where(builder.and(
                        builder.between(filteredRoot.<Date>get("endDate"),
                                startDate, endDate),
                        builder.equal(filteredRoot.get("successfullyFinished"),
                                successfullyFinished)));
        Long filteredMenteesCount = getEntityManager()
                .createQuery(queryForFiltering).getSingleResult();
        ...
        Long allMenteesCount = getEntityManager().createQuery(queryForCountAll)
                .getSingleResult();
