...
public class LoggingAspect {
    ...
    public Object aroundLog(final ProceedingJoinPoint joinPoint)
            throws Throwable {
        String methodName = AspectHelper.getMethodName(joinPoint);
        LOG.info("Invokation started: "
                + joinPoint.getTarget().getClass().getName() + " -> "
                + methodName);
        Object returnValue = joinPoint.proceed();
        LOG.info("Invokation finished: "
                + joinPoint.getTarget().getClass().getName() + " -> "
                + methodName);
        return returnValue;
    }
    ...
    @AfterThrowing(
            pointcut = "daoClasses() && annotation()", throwing = "ex")
    public void afterException(final JoinPoint joinPoint, final Exception ex) {
        String methodName = AspectHelper.getMethodName(joinPoint);
        LOG.error("Exception was thrown on "
                + joinPoint.getTarget().getClass().getName() + " -> "
                + methodName, ex);
    }
}
